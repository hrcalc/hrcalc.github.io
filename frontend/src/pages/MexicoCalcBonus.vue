<template>
  <v-container>
    <v-responsive class="align-center" min-width="1180">
      <v-row>
        <v-col cols="4">
          ...
          <v-text-field label="Gross Salary" v-model.number="gross_salary" type="number" hide-details
            prefix="$"></v-text-field>
          <v-text-field label="Annual Bonus (Multiplier)" v-model.number="annual_bonus_multiplier" type="number" hide-details
            prefix="Salary x"></v-text-field>
          ...
          <v-table density="compact">
            <thead>
              <tr>
                ...
                <th class="text-left">Annual Bonus</th>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="text-left">$ {{ gross_salary }}</td>
                <td class="text-left">$ {{ rnd(annual_bonus) }}</td>
...
let annual_bonus_multiplier = $ref(0)
let annual_bonus = $computed(() => annual_bonus_multiplier*gross_salary)
let net_salary_expected = $ref(0)
let exchange_rate = $ref(21)
...
let bonus_isr_tax = $computed(() => calculate_isr(gross_salary + annual_bonus)-calculate_isr(gross_salary))
...
let christmas_net_salary = $computed(() => {
  return gross_salary + christmas_bonus + annual_bonus - isr_tax - christmas_actual_tax - imss_tax - bonus_isr_tax
})
